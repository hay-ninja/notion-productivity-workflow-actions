...
- **The Notion side is built** — tasks, internships, settings, digests, Home dashboard.
...
**Total time:** ~45 min. Step 4 (Google) is the only fiddly part — budget 15 min.
...
1. **notion.so/my-integrations → New integration** (Internal). Copy the secret →
   `NOTION_TOKEN`.
2. **Share all four databases with the integration** — the #1 thing people forget. If
   you skip it the robots see nothing and fail silently. For each of tasks,
   Internships, Automations Settings, Daily Digests: open it → `•••` → **Connections
   → Connect to →** your integration.
3. Add the four `NOTION_*_DB` secrets from `.env.example`.

> Checkpoint: you can now test the **dashboard** and **auto-archive** robots.

## Step 4 — Google (15 min) — for the digest + calendar sync
...
4. **developers.google.com/oauthplayground** → gear icon → check *Use your own OAuth
   credentials* → paste them. Authorize these scopes:
   ```
...
## Step 5 — Claude key (2 min)

**console.anthropic.com → API keys → Create key** → `ANTHROPIC_API_KEY`.
Optionally set `ANTHROPIC_MODEL` (defaults to a cheap Haiku model).

## Step 6 — Phone notifications (5 min)
...
## Step 7 — Test each robot, then let it run

**Actions tab → pick a workflow → Run workflow.** In this order:

1. **Dashboard** — proves Notion reads work.
2. **A · Email Digest** — a page appears in Daily Digests (evening runs add tomorrow's
   top 3).
3. **B · Deadline Reminders** — phone buzzes (or correctly sends nothing).
4. **C · GCal Sync** — make a test task with a due date; confirm the event appears and
   `GCal Event ID` fills in. **Then turn any old Zapier zap OFF.**
5. **D · Weekly Review** — tick the Notion checkbox, run, untick.
6. **E · Auto-Archive** — safe; only touches tasks Done > 7 days.
...
## Step 8 — Pin the chart in Notion
...
| Auth error on digest/calendar | Refresh token expired or wrong scopes — redo 4.4. |
...
- [ ] `NOTION_TOKEN` + all 4 databases shared
- [ ] `NOTION_TASKS_DB`, `NOTION_INTERNSHIPS_DB`, `NOTION_SETTINGS_DB`, `NOTION_DIGESTS_DB`
- [ ] `ANTHROPIC_API_KEY`
